...
const toIdSchema = (schema, id, definitions, formData = {}, idPrefix = "root") => {
    const idSchema = {
      $id: id || idPrefix,
    };
    if ("$ref" in schema) {
...
function DefaultTemplate(props) {
  ...
  return (
    <div className={classNames}>
      {false && displayLabel && <Label label={label} required={required} id={id} />}
      {false && displayLabel && description ? description : null}
      {children}
      {false && errors}
      {false && help}
    </div>
...
function SchemaFieldRender(props) {
  ...
  const _required = required === true || schema.required === true;
  if ((requiredOnly === true && !_required) || Object.keys(schema).length === 0) {
    // See #312: Ensure compatibility with old versions of React.
    return <div />;
  }
