...
function DefaultArrayItem(props) {
  const {IconButton} = props.widgets;

  return (
    <div key={props.index} style={{
          minWidth: "300px",
    }} className={`${props.hasToolbar ? "col col-xs-9" : "col col-xs-12"} ${props.className}`}>
      <div style={{
          ...
          {props.hasToolbar && (
            <div className='btn-toolbar array-item-toolbox'>
              <div
  ...
  return (
    ...
    {(props.uiSchema['ui:description'] || props.schema.description) && (
      ...
      <div
        ...
        {props.items && props.items.map((p) => DefaultArrayItem({...props, ...p}))}
      </div>
...
class ArrayField extends Component {
  ...
  renderNormalArray() {
    ...
    const arrayProps = {
      ...
      items: formData.map((item, index) => {
        ...
        return this.renderArrayFieldItem({
          ...others,
          index,
          canRemove: !schema.minItems || schema.minItems < formData.length,
          canMoveUp: index > 0,
    ...
    return <Component registry={registry} {...arrayProps} widgets={registry.widgets}/>;
  }
  ...
  renderFixedArray() {
    ...
    return <Template {...arrayProps} widgets={registry.widgets}/>;
  }
...
function AddButton({ onClick, disabled, widgets }) {
  const {IconButton} = widgets;
  return (
    <div className='btn-toolbar' role="toolbar" aria-label="Button toolbar actions">
      <div className='btn-group btn-group-justified' role="group" aria-label="Button group actions">
        <IconButton
          primary
          tabIndex={0}
          onClick={onClick}
          label='Add'
          fullWidth
          disabled={disabled}>add</IconButton>
      </div>
    </div>
  );
}
